...
const Albums = () => {
  ...
  const togglePhotos = async (albumId) => {
    if (visiblePhotos[albumId]) {
      // If photos are already visible, hide them
      setVisiblePhotos((prev) => ({ ...prev, [albumId]: 0 }));
    } else {
      // If photos are not yet loaded, load them
      if (!photos[albumId]) {
        try {
          const response = await fetch(
      ...
      <ul>
        {albums.map((album) => (
          <li key={album.id}>
            ...
            <button className="button-b" onClick={() => togglePhotos(album.id)}>
            ...
            {visiblePhotos[album.id] && (
              <div>
                <div style={{ display: "flex", flexWrap: "wrap" }}>
                  {photos[album.id]
                    ?.slice(0, visiblePhotos[album.id])
                    .map((photo) => (
                      <img
                        key={photo.id}
                ...
                {visiblePhotos[album.id] < photos[album.id]?.length && (
                  <button className="button-b" onClick={() => showMorePhotos(album.id)}>המשך</button>
                )}
              </div>
            )}
          </li>
